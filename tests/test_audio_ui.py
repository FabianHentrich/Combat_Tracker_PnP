...
def audio_ui_module(mock_tkinter, mock_pygame):
    ...
    import src.ui.audio_player_view
    import src.ui.audio_settings_view
    yield src.ui.audio_player_view, src.ui.audio_settings_view
...
def test_player_widget_toggle_play(root, controller, audio_ui_module):
    ...
    widget.toggle_play()
    controller.pause.assert_called()

    # Test Unpause
    controller.is_paused = True
    widget.toggle_play()
    controller.pause.assert_called() # pause() toggles pause state
...
def test_player_widget_mute(root, controller, audio_ui_module):
    ...
    controller.set_volume.assert_called_with(0)
    ...
    assert widget.is_muted is False
    assert widget.vol_var.get() > 0
    controller.set_volume.assert_called()
...
def test_settings_window_add_file(root, controller, audio_ui_module):
    ...
    with patch('src.ui.audio_settings_view.filedialog') as mock_filedialog, \
         patch('src.ui.audio_settings_view.ScrollableFrame', MockScrollableFrame), \
         patch('src.ui.audio_settings_view.ToolTip', MagicMock()):
...
def test_track_card_actions(root, controller, audio_ui_module):
    ...
    with patch('src.ui.audio_settings_view.ToolTip', MagicMock()):
