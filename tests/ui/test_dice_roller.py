...
def clean_imports():
    ...
    # Ensure we have a clean src.ui.components.dice_roller
    if 'src.ui.components.dice_roller' in sys.modules:
        importlib.reload(sys.modules['src.ui.components.dice_roller'])
    else:
...
def test_dice_roller_integration(mock_root):
    ...
    print(f"DEBUG: DiceRoller is {src.ui.components.dice_roller.DiceRoller}")

    # We need to mock the UI creation parts because they require a real tk environment
    with patch('src.ui.components.dice_roller.DiceRoller._create_ui'), \
         patch('tkinter.ttk.LabelFrame'), \
         patch('tkinter.StringVar'), \
         patch('src.ui.components.dice_roller.roll_exploding_dice', return_value=(10, [6, 4])) as mock_roll:
...
def test_dice_roller_no_explosion(mock_root):
     ...
     with patch('src.ui.components.dice_roller.DiceRoller._create_ui'), \
         patch('tkinter.ttk.LabelFrame'), \
         patch('tkinter.StringVar'), \
         patch('src.ui.components.dice_roller.roll_exploding_dice', return_value=(3, [3])) as mock_roll:
