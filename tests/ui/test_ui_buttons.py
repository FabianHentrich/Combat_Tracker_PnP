...
def test_btn_add_character(app):
    ...
    app.view.get_quick_add_data.return_value = {
        "name": "TestChar", "lp": 20, "rp": 5, "sp": 0, "init": 10,
        "gew": 2, "level": 0, "type": CharacterType.PLAYER, "surprise": False
    }
...
def test_btn_deal_damage(app):
    """Testet den 'Schaden' Button mit der neuen Mehrfachauswahl-Logik."""
    c1 = Character("Target", 20, 0, 0, 10, char_id="uuid-1")
    app.engine.add_character(c1) # Use engine's method to add char

    # Mock Selection (Plural)
    app.view.get_selected_char_ids.return_value = ["uuid-1"]
    # Mock Input
...
def test_btn_add_status(app):
    ...
    app.view.get_status_input.return_value = {
        "status": "Vergiftung", "duration": "3", "rank": "1"
    }
...
def test_btn_delete_character(app):
    ...
    # Wichtige Änderung: Der CharacterManagementHandler verwendet get_management_target
    # und get_selected_char_id (Singular) für gezielte Aktionen.
    app.view.get_management_target.return_value = "Ausgewählter Charakter"
    app.view.get_selected_char_id.return_value = "uuid-1"

    app.character_handler.manage_delete()
...
def test_btn_undo_redo(app):
    ...
    c1 = Character("A", 10, 0, 0, 10, char_id="uuid-1")
    app.engine.add_character(c1)
    ...
    app.combat_handler.deal_damage()
    assert app.engine.get_character_by_id("uuid-1").lp == 5

    app.history_manager.undo()
    assert app.engine.get_character_by_id("uuid-1").lp == 10
